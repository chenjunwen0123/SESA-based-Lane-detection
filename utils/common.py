...
def get_train_loader(cfg):
    if cfg.dataset == 'CULane':
        ...
        train_loader = TrainCollect(cfg.batch_size, 4, cfg.data_root,
                                    os.path.join(cfg.data_root, 'train', 'train_gt.txt'), get_rank(), get_world_size(),
                                    cfg.row_anchor, cfg.col_anchor, cfg.train_width, cfg.train_height, cfg.num_cell_row,
                                    cfg.num_cell_col, cfg.dataset, cfg.crop_ratio)
    else:
...
def inference_culane_tusimple(net, data_label):
    ...
    res_dict = {'cls_out': pred['loc_row'], 'cls_label': data_label['labels_row'], 'cls_out_col': pred['loc_col'],
                ...
                'cls_out_col_ext_label': cls_out_col_ext_label, 'labels_row_float': data_label['labels_row_float'],
...
def inference_curvelanes(net, data_label):
    ...
    res_dict = {'cls_out': pred['loc_row'], 'cls_label': data_label['labels_row'], 'cls_out_col': pred['loc_col'],
                ...
                'cls_out_ext': pred['exist_row'], 'cls_out_ext_label': cls_out_ext_label,
                'cls_out_col_ext': pred['exist_col'],
                'cls_out_col_ext_label': cls_out_col_ext_label, 'seg_label': data_label['seg_images'],
                'seg_out_row': pred['lane_token_row'], 'seg_out_col': pred['lane_token_col']}
    if 'seg_out' in pred.keys():
        res_dict['seg_out'] = pred['seg_out']
